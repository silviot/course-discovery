...
class PersonInfo(object):
    def __init__(self, partner, uuid, target_uuid='', delete_only=False):
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            metavar=_('PERSON'),
            nargs='*',
            help=_('People to delete, in UUID:TARGET_UUID form. If delete-only is True, only UUID is required.'),
        )
        parser.add_argument(
            '--partner-code',
            metavar=_('PARTNER'),
        ...
        parser.add_argument(
            '--delete-only',
            help=_('Boolean to distinguish between only deleting people or deleting and moving information \
                to a target person.')
        )
        parser.add_argument(
            '--args-from-database',
    ...
    def parse_person(self, partner, person_str, delete_only=False):
        if delete_only:
            return PersonInfo(partner, person_str, delete_only=delete_only)
    ...
    @transaction.atomic
    def delete_person(self, pinfo, commit=False, delete_only=False):
    ...
    def delete_person_dups(self, options):
        ...
        pinfos = [self.parse_person(partner, p, delete_only) for p in options['people']]
        commit = options['commit']

        for info in pinfos:
            self.delete_person(info, commit=commit, delete_only=delete_only)
